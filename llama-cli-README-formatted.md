...
> A practical guide to installing, configuring, and running local GGUF LLMs with `llama-cli` across macOS, Windows, and Linux.
...
`llama-cli` is the command-line interface provided by the [`llama.cpp`](https://github.com/ggml-org/llama.cpp) ecosystem. It lets you run quantized **GGUF** models locally on consumer hardware, without sending prompts to a remote inference service.

### What this guide covers

- Installation on **macOS, Windows, and Linux**
- CPU, NVIDIA CUDA, AMD ROCm/HIP, Vulkan, and Apple Metal builds
- Running models from **Hugging Face** or local `.gguf` files
- Common `llama-cli` command-line options
- Basic hardware optimization and troubleshooting

---

## Installation

Choose the section for your operating system. If you have a dedicated NVIDIA or AMD GPU, the source-build options provide the most control over hardware acceleration.
...
You can run models directly from [Hugging Face](https://huggingface.co/) without manually downloading the model file first by using `-hf`:
...
```bash
llama-cli -hf unsloth/Llama-3.2-1B-Instruct-GGUF:Q4_0 -c 4096 -b 128 -ub 64 -t 6 -cnv
```

### MacBook Neo example

```bash
llama-cli -hf unsloth/Llama-3.2-1B-Instruct-GGUF:Q4_0 -c 8192 -b 128 -ub 64 -t 6 -cnv
```
...
> **Note:** The training workflow below is separate from using `llama-cli` for local inference and is included as reference material.
